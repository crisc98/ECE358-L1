...
class Simulator 
{
    ...
    bool arrival(long tick) {
        if (tick >= nextTick) {
            ...
            nextTick = tick + (long)(randomVariable * 1000000.0);
    ...
    bool departure(long tick) {
        ...
        if (tick == packet.getDepartureTick() + serviceTime) {
            // service time has elapsed
            queue.erase(queue.begin());

            long queueingDelay = packet.getDepartureTick() - packet.getArrivalTick();
